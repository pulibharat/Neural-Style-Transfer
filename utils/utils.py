...
class ImageFolderDataset(Dataset):
    def __init__(self, root, transform=None):
        ...
        self.files = [p for p in self.files if p.endswith(
            ('.jpg', '.png', '.jpeg'))]
...
def calc_mean_std(feat, eps=1e-5):
    ...
    feat_mean = feat.view(batch_size, channels, -
                          1).mean(dim=2).view(batch_size, channels, 1, 1)
    feat_var = feat.view(batch_size, channels, -
                         1).var(dim=2, unbiased=False) + eps
